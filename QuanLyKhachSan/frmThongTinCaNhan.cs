...
namespace QuanLyKhachSan
{
    public partial class frmThongTinCaNhan : Form
    {
        ...
        {
            TaiKhoan staff = TaiKhoanDAO.Instance.LoadStaffInforByUserName(username);
            lblUserName.Text = txbUserName.Text = staff.UserName;
            lblDisplayName.Text = txbDisplayName.Text = staff.DisplayName;
            txbStaffType.Text = LoaiTaiKhoanDAO.Instance.GetStaffTypeByUserName(username).Name;
        ...
        {
            TaiKhoanDAO.Instance.UpdateDisplayName(username, displayname);
        }
        public void UpdatePassword(string username, string password)
        {
            TaiKhoanDAO.Instance.UpdatePassword(username, password);
        }
        public void UpdateInfo(string username, string address, int phonenumber, string idCard, DateTime dateOfBirth, string sex)
        {
            TaiKhoanDAO.Instance.UpdateInfo(username, address, phonenumber, idCard, dateOfBirth, sex);
        }
        ...
        {
            if (TaiKhoanDAO.Instance.HashPass(txbPass.Text) == Password && txbNewPass.Text != String.Empty && txbReNewPass.Text != String.Empty)
            {
        ...
        private void bunifuThinButton21_Click(object sender, EventArgs e)
        {
            ...
            {
                if (TaiKhoanDAO.Instance.IsIdCardExists(txbIDCard.Text))
                {
