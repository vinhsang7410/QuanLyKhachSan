...
namespace QuanLyKhachSan
{
    public partial class frmQuyDinh : Form
    {
        ...
        private void UpdateSurcharge()
        {
            ...
            bool isFill = frmQuanLyKhachHang.CheckFillInText(new Control[] { txbValue });
            if (isFill)
            {
                QuyDinh surchargePre = groupParameter.Tag as QuyDinh;
                try
                {
                    QuyDinh surchargeNow = GetSurchargeNow();
                    ...
                    {
                        bool check = QuyDinhDAO.Instance.UpdateParameter(surchargeNow);
                        if (check)
                        {
        ...
        private void ChangeText(DataGridViewRow row)
        {
            ...
            else
            {
                ...
                QuyDinh parameter = new QuyDinh(((DataRowView)row.DataBoundItem).Row);
        ...
        {
            return QuyDinhDAO.Instance.LoadFullParameter();
        }
        private QuyDinh GetSurchargeNow()
        {
            frmQuanLyNhanVien.Trim(new Bunifu.Framework.UI.BunifuMetroTextbox[] { txbDescribe });
            return new QuyDinh(comboboxName.Text, double.Parse(txbValue.Text), txbDescribe.Text);
        }
        private DataTable GetSearchParameter()
        {
            return QuyDinhDAO.Instance.Search(txbSearch.Text);
        }
