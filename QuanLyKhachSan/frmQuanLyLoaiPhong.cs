...
namespace QuanLyKhachSan
{
    public partial class frmQuanLyLoaiPhong : Form
    {
        ...
        public frmQuanLyLoaiPhong(DataTable table)
        {
            InitializeComponent();
            TableRoomType = table;
        ...
        private LoaiPhong GetRoomTypeNow()
        {
        ...
        private DataTable GetSearchRoomType()
        {
            if (int.TryParse(txbSearch.Text, out int id))
                return LoaiPhongDAO.Instance.Search(txbSearch.Text, id);
            else
                return LoaiPhongDAO.Instance.Search(txbSearch.Text, 0);
        }
        ...
        private void UpdateRoomType()
        {
            ...
            if (!frmQuanLyKhachHang.CheckFillInText(new Control[] { txbName, txbPrice }))
            {
            ...
            {
                LoaiPhong roomTypePre = groupRoomType.Tag as LoaiPhong;
                try
                {
                    LoaiPhong roomTypeNow = GetRoomTypeNow();
                    ...
                    {
                        bool check = LoaiPhongDAO.Instance.UpdateRoomType(roomTypeNow, roomTypePre);
                        if (check)
                        {
        ...
        private void ChangeText(DataGridViewRow row)
        {
            ...
            else
            {
                ...
                LoaiPhong roomType = new LoaiPhong(((DataRowView)row.DataBoundItem).Row);
