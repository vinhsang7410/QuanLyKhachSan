...
namespace QuanLyKhachSan
{
    ...
    {
        #region Properties
        public static readonly string HassPass = "e10adc3949ba59abbe56e057f20f883e"; // pass mặc định
        #endregion
        ...
        {
            if (frmQuanLyKhachHang.CheckFillInText(new Control[] { txbUserName }))
            {
        ...
        {
            bool isFill = frmQuanLyKhachHang.CheckFillInText(new Control[] { txbUserName, comboBoxStaffType, txbName ,
            ...
            {
                TaiKhoan accountPre = groupStaff.Tag as TaiKhoan;
                try
                {
                    TaiKhoan accountnow = GetStaffNow();
                    ...
                    {
                        bool check = TaiKhoanDAO.Instance.UpdateAccount(accountnow);
                        if (check)
                        {
        ...
        private void ResetPassword()
        {
            try
            {
                bool check = TaiKhoanDAO.Instance.ResetPassword(txbUserName.Text, HassPass);
                if (check)
                {
        ...
        private DataTable GetSearchStaff()
        {
            if (int.TryParse(txbSearch.Text, out int phoneNumber))
                return TaiKhoanDAO.Instance.Search(txbSearch.Text, phoneNumber);
            else
                return TaiKhoanDAO.Instance.Search(txbSearch.Text, -1);
        }
        ...
        {
            frmQuyenTruyCap f = new frmQuyenTruyCap();
            f.ShowDialog();
            LoadFullStaffType();
            if (btnCancel.Visible == false)
                LoadFullStaff(GetFullStaff());
