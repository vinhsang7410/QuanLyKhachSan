...
namespace QuanLyKhachSan
{
    ...
    {
        #region Properties
        private frmQuanLyLoaiPhong _fRoomtType;
        #endregion
        ...
        private void LoadFullRoomType()
        {
            ...
            _fRoomtType = new frmQuanLyLoaiPhong(table);
            txbLimitPerson.DataBindings.Add(new Binding("Text", comboBoxRoomType.DataSource, "limitPerson"));
        }
        #endregion
        ...
        private void UpdateRoom()
        {
            ...
            if (!frmQuanLyKhachHang.CheckFillInText(new Control[] { txbNameRoom, comboBoxStatusRoom, comboBoxRoomType }))
            {
            ...
            {
                Phong roomPre = groupRoom.Tag as Phong;
                try
                {
                    ...
                    {
                        bool check = PhongDAO.Instance.UpdateCustomer(roomNow);
                        if (check)
                        {
        ...
        {
            return PhongDAO.Instance.LoadFullRoom();
        }
        ...
        {
            return TrangThaiPhongDAO.Instance.LoadFullStatusRoom();
        }
        ...
        private DataTable GetSearchRoom()
        {
            if (int.TryParse(txbSearch.Text, out int id))
                return PhongDAO.Instance.Search(txbSearch.Text, id);
            else
                return PhongDAO.Instance.Search(txbSearch.Text, 0);
        }
