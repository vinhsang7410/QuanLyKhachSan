...
namespace QuanLyKhachSan
{
    public partial class frmChiTietDatPhong : Form
    {
        int idBookRoom;
        string idCard;
        public frmChiTietDatPhong(int _idBookRoom,string _idCard)
        {
        ...
        {
            DataRow data = DatPhongDAO.Instance.ShowBookRoomInfo(idBookRoom);
            dpkDateCheckIn.Value = (DateTime)data["DateCheckIn"];
            dpkDateCheckOut.Value = (DateTime)data["DateCheckOut"];

            cbRoomType.Text = LoaiPhongDAO.Instance.GetRoomTypeByIdBookRoom(idBookRoom).Name;
        ...
        {
            DatPhongDAO.Instance.UpdateBookRoom(idBookRoom, (cbRoomType.SelectedItem as LoaiPhong).Id, dpkDateCheckIn.Value, dpkDateCheckOut.Value);
        ...
        private void bunifuThinButton23_Click(object sender, EventArgs e)
        {
            ...
            {
                if (DatPhongDAO.Instance.IsIDBookRoomExists(idBookRoom))
                {
                    DatPhongDAO.Instance.DeleteBookRoom(idBookRoom);
