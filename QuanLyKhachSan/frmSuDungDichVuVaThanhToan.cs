...
namespace QuanLyKhachSan
{
    ...
    {
        string staffSetUp;
        public frmSuDungDichVuVaThanhToan(string userName)
        {
            staffSetUp = userName;
            InitializeComponent();
            LoadData();
        ...
        {
            HoaDonDAO.Instance.UpdateRoomPrice(idBill);
            HoaDonDAO.Instance.UpdateServicePrice(idBill);
            HoaDonDAO.Instance.UpdateOther(idBill, discount);

        }
        public void LoadListRoomType()
        {
            List<LoaiPhong> roomTypes = LoaiPhongDAO.Instance.LoadListRoomType();
        ...
        {
            cbServiceType.DataSource = LoaiDichVuDAO.Instance.GetServiceTypes();
            cbServiceType.DisplayMember = "Name";
        }
        public void LoadListService(int idServiceType)
        {
            cbService.DataSource = DichVuDAO.Instance.GetServices(idServiceType);
            cbService.DisplayMember = "Name";
        }
        public void DrawControl(Phong room, Bunifu.Framework.UI.BunifuTileButton button)
        {
            int idRoomTypeName = LoaiPhongDAO.Instance.GetRoomTypeByIdRoom(room.Id).Id;
        ...
        public void LoadListFullRoom()
        {
            ...
            List<Phong> rooms = PhongDAO.Instance.LoadListFullRoom();
            foreach (Phong item in rooms)
            {
                ...
                button.Image = global::QuanLyKhachSan.Properties.Resources.Room;
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            {
                if (item != button)
                    DrawControl((item as Bunifu.Framework.UI.BunifuTileButton).Tag as Phong, item as Bunifu.Framework.UI.BunifuTileButton);
            }
            Phong room = button.Tag as Phong;
            ShowBill(room.Id);
            if (!HoaDonDAO.Instance.IsExistsBill(room.Id))
            {
                int idReceiveRoom = NhanPhongDAO.Instance.GetIdReceiveRoomFromIdRoom(room.Id);
                InsertBill(idReceiveRoom, staffSetUp);
            }
            HoaDonDAO.Instance.UpdateRoomPrice(HoaDonDAO.Instance.GetIdBillFromIdRoom(room.Id));
        ...
        {
            return HoaDonDAO.Instance.IsExistsBill(idRoom);
        }
        public bool IsExistsBillDetails(int idRoom, int idService)
        {
            return ChiTietHoaDonDAO.Instance.IsExistsBillDetails(idRoom, idService);
        }
        public bool InsertBill(int idReceiveRoom, string staffSetUp)
        {
            return HoaDonDAO.Instance.InsertBill(idReceiveRoom, staffSetUp);
        }
        public bool InsertBillDetails(int idBill, int idService, int count)
        {
            return ChiTietHoaDonDAO.Instance.InsertBillDetails(idBill, idService, count);
        }
        public bool UpdateBillDetails(int idBill, int idService, int _count)
        {
            return ChiTietHoaDonDAO.Instance.UpdateBillDetails(idBill, idService, _count);
        }
        public void AddBill(int idRoom, int idService, int count)
        {
            if (IsExistsBill(idRoom))
            {
                ...
                if (!IsExistsBillDetails(idRoom, idService))
                {
                    ...
                    if (count > 0)
                    {
                        int idBill = HoaDonDAO.Instance.GetIdBillFromIdRoom(idRoom);
            ...
            else
            {
                ...
                if (count > 0)
                {
                    int idReceiveRoom = NhanPhongDAO.Instance.GetIdReceiveRoomFromIdRoom(idRoom);
                    InsertBill(idReceiveRoom, staffSetUp);
                    int idBill = HoaDonDAO.Instance.GetIdBillMax();
                    InsertBillDetails(idBill, idService, count);
                }
                else
                    MetroFramework.MetroMessageBox.Show(this, "Số lượng không hợp lệ.\nVui lòng nhập lại.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            listViewUseService.Items.Clear();
            DataTable dataTable = HoaDonDAO.Instance.ShowBill(idRoom);
        ...
        public void ShowBillRoom(int idRoom)
        {
            ...
            if (IsExistsBill(idRoom))
            {
                DataRow data = HoaDonDAO.Instance.ShowBillRoom(idRoom);
        ...
        {
            LoadListService((cbServiceType.SelectedItem as LoaiDichVu).Id);
        }
        ...
        private void cbService_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbService.SelectedItem != null)
                txbPrice.Text = (cbService.SelectedItem as DichVu).Price.ToString("c0", cultureInfo);
        }

        private void btnAdd_Click(object sender, EventArgs e)
        {
            totalPrice = 0;
            Phong room = flowLayoutRooms.Tag as Phong;
            AddBill(room.Id, (cbService.SelectedItem as DichVu).Id, int.Parse(numericUpDownCount.Value.ToString()));
            ShowBill(room.Id);
        ...
        private void btnAddCustomer_Click(object sender, EventArgs e)
        {
            ...
            {
                int idBill = HoaDonDAO.Instance.GetIdBillFromIdRoom(room.Id);
                Pay(idBill, int.Parse(numericUpDown1.Value.ToString()));
                BaoCaoDAO.Instance.InsertReport(idBill);
                MessageBox.Show("Thanh toán thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Hide();
                frmInHoaDon fPrintBill = new frmInHoaDon(room.Id, idBill);
