...
namespace QuanLyKhachSan
{
    ...
    {
        #region Properties
        frmQuanLyLoaiDichVu _fServiceType;
        #endregion
        ...
        private void LoadFullServiceType()
        {
            ...
            _fServiceType = new frmQuanLyLoaiDichVu(table);
        }
        #endregion
        ...
        private void ChangeText(DataGridViewRow row)
        {
            ...
            else
            {
                ...
                DichVu room = new DichVu(((DataRowView)row.DataBoundItem).Row);
        ...
        private void UpdateService()
        {
            ...
            if (!frmQuanLyKhachHang.CheckFillInText(new Control[] { txbName, comboBoxServiceType, txbPrice }))
            {
            ...
            {
                DichVu servicePre = groupService.Tag as DichVu;
                try
                {
                    DichVu serviceNow = GetServiceNow();
                    ...
                    {
                        bool check = DichVuDAO.Instance.UpdateService(serviceNow, servicePre);
                        if (check)
                        {
        ...
        private DataTable GetSearchService()
        {
            if (int.TryParse(txbSearch.Text, out int id))
                return DichVuDAO.Instance.Search(txbSearch.Text, id);
            else
                return DichVuDAO.Instance.Search(txbSearch.Text, 0);
        }
        #endregion
